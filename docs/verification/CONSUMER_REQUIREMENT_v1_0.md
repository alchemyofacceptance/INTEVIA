# CONSUMER-SIDE REQUIREMENT, STATED EXACTLY — v1.0
    ...
    Supersedes: v0.9, v0.8, v0.7 and v0.6 (none adopted) and, on adoption, v0.5.
...
### 0.0b C10 — one authoritative implementation (NEW in v1.0)
...
| `summary.json` → `identity` | `commit`, **`attested_commit` (C1)**, **`commit_tree`, `working_tree_git_tree` (C5)** | `required binding field <f> is absent from the coordinator summary identity record (summary.json identity): refused before any comparison` |
...
fallback** for any field; a field present with the wrong type is treated by §3, not silently coerced.
...
2. find its row for `verification/mutations.py`;
...
| `tests.by_outcome` has the single key `FAIL` | the target failed, and failed only |
